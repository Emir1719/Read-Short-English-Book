...
class StorySelectFontSize extends StatelessWidget {
  const StorySelectFontSize({super.key});
            ...
            Row(
              ...
              children: [
                SelectFontButton(
                  icon: Icons.remove,
                  onPressed: () {
                    if (selectedStyle.fontSize! > 15) {
                      // Decrease font size and send the updated style to the bloc
                      TextStyle newStyle =
                          selectedStyle.copyWith(fontSize: selectedStyle.fontSize! - 1);
                      bloc.add(ChangeStyle(newStyle));
                    }
                  },
                ),
                Text(
                  selectedStyle.fontSize?.toStringAsFixed(0) ?? "16",
                  style: context.text.bodyLarge?.copyWith(fontWeight: FontWeight.normal),
                ),
                SelectFontButton(
                  icon: Icons.add,
                  onPressed: () {
                    if (selectedStyle.fontSize! < 30) {
